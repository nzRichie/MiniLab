#!/bin/sh
# Starter config for NEWY, applied by spawn.sh and re-applied by reset.sh.
#
# NEWY arrives with NOTHING configured, because configuring it is the lab. Its
...
#   port_LOND    109.0.1.2/30   (toward LOND)
#   port_ZURI    109.0.2.1/30   (toward ZURI)
#   port_PARI    109.0.6.2/30   (toward PARI)
#   host         109.104.0.2/24   (toward its own host)
#   lo           109.154.0.1/32   (loopback, also the router-id)
#
...
LAB_IFS="port_LOND port_ZURI port_PARI host"
...
if [ $ready -ne 1 ]; then
    echo "FRR did not come back up on newy" >&2
    exit 1
fi
