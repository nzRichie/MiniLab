#!/bin/sh
# Starter config for PARI, applied by spawn.sh and re-applied by reset.sh.
#
# PARI arrives with NOTHING configured, because configuring it is the lab. Its
...
#   port_TRGA    109.0.4.1/30   (toward TRGA)
#   port_LOND    109.0.5.2/30   (toward LOND)
#   port_NEWY    109.0.6.1/30   (toward NEWY)
#   host         109.102.0.2/24   (toward its own host)
#   lo           109.152.0.1/32   (loopback, also the router-id)
#
...
LAB_IFS="port_TRGA port_LOND port_NEWY host"
...
if [ $ready -ne 1 ]; then
    echo "FRR did not come back up on pari" >&2
    exit 1
fi
