#!/bin/sh
# Starter config for LOND, applied by spawn.sh and re-applied by reset.sh.
#
# LOND arrives with NOTHING configured, because configuring it is the lab. Its
...
#   port_NEWY    109.0.1.1/30   (toward NEWY)
#   port_PARI    109.0.5.1/30   (toward PARI)
#   host         109.101.0.2/24   (toward its own host)
#   lo           109.151.0.1/32   (loopback, also the router-id)
#
...
LAB_IFS="port_NEWY port_PARI host"
...
if [ $ready -ne 1 ]; then
    echo "FRR did not come back up on lond" >&2
    exit 1
fi
