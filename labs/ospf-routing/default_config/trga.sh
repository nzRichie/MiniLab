#!/bin/sh
# Starter config for TRGA, applied by spawn.sh and re-applied by reset.sh.
#
# TRGA arrives with NOTHING configured, because configuring it is the lab. Its
...
#   port_ZURI    109.0.3.1/30   (toward ZURI)
#   port_PARI    109.0.4.2/30   (toward PARI)
#   host         109.103.0.2/24   (toward its own host)
#   lo           109.153.0.1/32   (loopback, also the router-id)
#
...
LAB_IFS="port_ZURI port_PARI host"
...
if [ $ready -ne 1 ]; then
    echo "FRR did not come back up on trga" >&2
    exit 1
fi
