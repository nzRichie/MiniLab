...
#
# Scoped to the seven containers named in lib.sh plus the wiring helper. It must
# never call platform/cleanup/cleanup.sh or hard_reset.sh: those wipe the whole
...
# container, so removing the containers removes all of it. The iptables rules the
# learner wrote live in the server container's own network namespace and go the
# same way. The helper is removed too, in case an interrupted spawn left one
# behind.
set -uo pipefail
...
for ctn in "$SERVER_CTN" "$CLIENT_CTN" "$RTR_CTN" "$ATTACKER_CTN" "$CUSTOMER_CTN" \
           "$SW_IN_CTN" "$SW_OUT_CTN" "$HELPER_CTN"; do
