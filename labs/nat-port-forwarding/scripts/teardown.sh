...
# the interfaces go with them, and every nftables rule the learner wrote goes with
# the router's network namespace. Scoped to THIS lab only — deliberately does NOT
# call platform/cleanup/cleanup.sh, which wipes the whole mini-internet and
# deletes groups/.
set -uo pipefail
...
# container this lab started under a name lib.sh no longer knows still goes.
mapfile -t ctns < <( docker ps -aq --filter "name=^${AS}_L3_${DC}_" )
