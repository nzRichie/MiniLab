...
# handout.tex \input{}s two more files, so either of them being newer than the PDF
# also means the PDF is stale. topology.tex is generated from scripts/topology.sh
# by labs/tools/topofig.py, which is an authoring tool and is not shipped; this
# only rebuilds the PDF from whatever topology.tex is already here.
stale=0
[ -f "$pdf" ] || stale=1
