#!/usr/bin/env bash
# Spawn the DNS tunnelling and egress control lab: a resolver and two
# workstations on an inside segment, a gateway, and two authoritative servers on
# an outside segment. Self-contained -- drives docker + the veth primitives
# directly, WITHOUT the full platform/startup.sh pipeline.
#
#   ws1 ----\                          /-- pub        (119.1.0.10)
#   ws2 -----[ S1 ] --- gw --- [ S2 ]-
#   resolver /                         \-- collector   (119.1.0.66)
#
...
# which is what makes a capture taken there a complete record of what leaves, and
# what makes the gateway the one place an egress policy can be enforced.
#
# The gateway forwards and does not translate addresses, so every query crossing
# it still carries the inside host's own source address.
set -euo pipefail
...
# The starter policy and Part 2A both register an nftables chain at the forward
# hook with a connection-state match. Registering a base chain needs the
# nf_tables machinery in the container's namespace and the state match needs
# nf_conntrack; autoloading either needs privilege in the initial namespace,
# which a rootless daemon's containers do not have. Fail here with the reason
# rather than halfway through applying the gateway's starter config.
...
if ! docker run --rm --cap-add=NET_ADMIN "$HOST_IMAGE" sh -c \
    ...
    echo "match inside a container. The starter policy and Part 2A need one. The" >&2
    echo "nf_tables, nft_chain_filter and nf_conntrack modules must be loaded on the" >&2
    echo "host; any machine already running Docker networking normally has them." >&2
    exit 1
fi
...
# --init everywhere: PID 1 is `sleep infinity`, which never reaps, and named,
# the decoder and every dig the learner runs fork children. ip_forward is set at
# creation because Docker mounts /proc/sys read-only in an unprivileged
# container; it is on for the gateway and explicitly off everywhere else.
log "starting gateway $GW_CTN (two segments, forwarding on, DNS-only egress)"
...
# Apply the per-device starter configs in lib.sh order: the outside
# authoritative servers first so there is something to resolve, then the
# resolver, then the gateway, then the workstations.
for d in "${DEVICES[@]}"; do
    ctn="$( ctn_of "$d" )"
