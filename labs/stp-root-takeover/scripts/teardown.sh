...
# the OVS ports (the ring links and the four host links) go with them. Scoped to
...
docker rm -f "$SW1_CTN" "$SW2_CTN" "$SW3_CTN" \
             "$ATTACKER_CTN" "$VICTIM_CTN" "$GATEWAY_CTN" 2>/dev/null || true
