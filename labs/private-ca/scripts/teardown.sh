...
# the host-side interfaces go with them. Scoped to THIS lab only — deliberately
# does NOT call platform/cleanup/cleanup.sh, which wipes the whole mini-internet
# and deletes groups/.
set -uo pipefail
...
mapfile -t ctns < <( docker ps -aq --filter "name=^${AS}_L7_${DC}_" )
