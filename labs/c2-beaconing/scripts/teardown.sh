...
#
# Scoped to the eight containers named in lib.sh plus the wiring helper. It must
...
removed=0
for ctn in "$GW_CTN" "$WS1_CTN" "$WS2_CTN" "$WS3_CTN" "$EXT1_CTN" "$EXT2_CTN" "$SW_IN_CTN" "$SW_OUT_CTN" "$HELPER_CTN"; do
