#!/bin/sh
# Starter config for host5: the DNS reflector, and an honest server.
#
# dnsmasq 2.90, authoritative for four names and nothing else. It runs no
# recursion, has no upstream and reads no hosts file. It answers the question it
# is asked, to the address the question says it came from, which is all a
# reflector has to do and is not a misconfiguration.
#
# The four names are the measurement Stage 1 opens on:
#
#   amplify.lab    one synthetic 250-byte TXT record
#   google.com     the real TXT record set, 15 records, captured 2026-07-28
#   spotify.com    the same, 35 records
#   amazon.com     the same, 61 records, which overflows the 4096-byte ceiling
#
# edns-packet-max=4096 is the one line that decides the size of the reply. A
# query that advertises a 4096-byte buffer draws the whole set; without this
# line every reply caps at 512 bytes and the amplification halves. dnsmasq packs
# as many records as fit and sets the TC flag rather than returning an empty
# answer, which is why amazon.com comes back at 4079 bytes with 47 of its 61
# records in it.
#
# This machine never attacks anything. It sits on the same segment as the four
# sources on purpose: a spoofed query never crosses the router, so everything
# the victim's network sees is reply traffic from a well-behaved server.
set -e
...
# 250 bytes of payload keeps the whole amplify.lab reply inside a single
# 512-byte UDP DNS message, so it answers a plain query too.
...
# hosts file. Serves amplify.lab plus three real domains' TXT sets (appended
# below).
port=53
no-resolv
no-hosts
log-queries
# Allow EDNS0 replies up to 4096 bytes over UDP. A plain 512-byte query still
# caps at 512; a query that advertises a larger buffer draws the bigger real
# answers, up to this ceiling. amazon.com's set exceeds 4096, so its reply
# truncates here (the TC flag is set) and the full answer only arrives over TCP.
...
# snapshot). dnsmasq returns every line that shares a name as a separate RR, so
# a query for the name draws the whole set back. A quoted heredoc keeps the data
...
# A plain web page on tcp/80 as well, because question 3 compares DNS and NTP
# with TCP as a reflector and the comparison has to be between protocols rather
# than between machines. Nothing about this service is unusual: a spoofed SYN
# arrives, the server answers SYN|ACK to the address the SYN claimed, and
# retransmits it when that address says nothing back.
mkdir -p /var/www/localhost/htdocs /var/log/lighttpd
cat > /var/www/localhost/htdocs/index.html <<'HTML'
<html><body><h1>129.1.0.88</h1><p>An ordinary server on the field segment.</p></body></html>
HTML
...
# Restart dnsmasq cleanly so a reset always comes up with a fresh listener.
...
echo "host5: DNS reflector up at ${HOST_IP} (amplify.lab + google/spotify/amazon TXT sets), web on tcp/80"
